	...
	this.zoomCamera = function () {
		...
		if ( _state === STATE.TOUCH_ZOOM_PAN ) {
			...
			if ( scope.object.isPerspectiveCamera ) {
				...
				console.log("zoom2 : "+scope.object.zoom)
			...
			if ( factor !== 1.0 && factor > 0.0 ) {
				...
				if ( scope.object.isPerspectiveCamera ) {
					if(factor < 1 && _eye.x < 0.05)
					{

						var direction = new Vector3();
						scope.object.getWorldDirection( direction );
						scope.object.position.add(direction.multiplyScalar( factor*0.05 ))
						scope.target.add( direction.multiplyScalar( factor*0.05 ));
